...
void condition( node *currentNode )
{
    ...
    else
    {
        ...
        emit( 2, curReg-1, 0, relOpCode );  // EQL thru GEQ
        curReg--;
    }
...
void expression( node *currentNode )
{
    ...
    while ( currentToken == plusSym || currentToken == minusSym )
    {
        ...
        if ( addop == plusSym )
        {
            emit ( 2, curReg-1, 0, 2 );    // ADD = 13
            curReg--;
        }
        if ( addop == minusSym )
        {
            emit ( 2, curReg-1, 0, 3 );    // SUB = 14
            curReg--;
        }
    }
...
void term( node *currentNode )
{
    ...
    while ( currentToken == slashSym || currentToken == multSym )
    {
        ...
        {
            emit( 2, curReg-1, 0, 4 ); // MUL = 15
            curReg--;

        }
        if ( mulOp == slashSym )    // Division
        {
            emit( 2, curReg-1, 0, 5 ); // DIV = 16
            curReg--;
        }
    }
}// end function term
